...
class EditRoomPhotoMobilePage extends StatefulWidget {
  final Room room;

   EditRoomPhotoMobilePage({super.key, required this.room});

  @override
  State<EditRoomPhotoMobilePage> createState() => _EditRoomPhotoMobilePageState();
...
class _EditRoomPhotoMobilePageState extends State<EditRoomPhotoMobilePage> {
  ...
  void _showContextMenu(BuildContext context, TapDownDetails details, int fileId) async {
    ...
    switch (result) {
      case 'del':
        BlocProvider.of<DeleteRoomPhotoCubit>(context).deleteRoomPhoto(fileId);
        break;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocListener<DeleteRoomPhotoCubit, DeleteRoomPhotoState>(
    listener: (context, state) {
    if(state is DeleteRoomPhotoSucceeded || state is DeleteRoomPhotoFailed) {
    if(_progressDialogShowing) {
    Navigator.of(context, rootNavigator: true).pop();
    _progressDialogShowing = false;
    }

    if (state is DeleteRoomPhotoSucceeded) {
    // setState((){
    //   photos = state.photos;
    // });
    ...
    } else if (state is DeleteRoomPhotoFailed) {
